export const BREW_CASK = (app: string) =>
    `yes | HOMEBREW_NO_AUTO_UPDATE=1 brew install --cask ${app}`
export const BREW_INSTALL = (formula: string) =>
    `yes | HOMEBREW_NO_AUTO_UPDATE=1 brew install ${formula}`
export const BREW_TAP = (tapTo: string) => `yes | HOMEBREW_NO_AUTO_UPDATE=1 brew tap ${tapTo}`
